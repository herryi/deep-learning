...
def batch_data(words, sequence_length, batch_size):
    ...
    while(count+sequence_length < len(words)):
        # feature tensors
        f = words[count:count+sequence_length]
        feature_tensors.append(f)
        
        # target tensors
        t = words[count+sequence_length]
        target_tensors.append(t)
        
        count += 1
    
    data = TensorDataset(torch.FloatTensor(feature_tensors), torch.FloatTensor(target_tensors))
...
class RNN(nn.Module):
    ...
    def forward(self, nn_input, hidden):
        ...
        embedding = self.embedding_layer(nn_input.long())
        ...
        # reshape into (batch_size, seq_length, output_size)
        output_view = output.view(batch_size, -1, self.output_size)
        # get last batch
        out = output_view[:, -1]
...
def forward_back_prop(rnn, optimizer, criterion, inp, target, hidden):
    ...
    # changing types
    if train_on_gpu:
        inpt = inp.type(torch.cuda.FloatTensor).detach()
        trgt = target.type(torch.cuda.LongTensor).detach()
    else:
        inpt = inp.type(torch.FloatTensor)
        trgt = target.type(torch.LongTensor)
...
# **答案:** (在这里写下) 开始尝试sequence_length=64和batch_size=64，发现模型收敛不够快。使用sequence_length=16和batch_size=128，模型收敛速度明显加快。根据前面的课程介绍，选择学习率0.001，2个隐藏层和256个隐藏单元，训练10次，很快得到模型loss小于3.5。
